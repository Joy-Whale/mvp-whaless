package me.whaless.app.common.widget.recyclerview.adapter;
...
public abstract class HeaderAdapter<T> extends AdapterPlus<T> {
			...
			if(isHasHeader() || isHasFooter()){
				...
				gridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
					...
					public int getSpanSize(int position) {
						return getItemViewType(position) == ITEM_TYPE_HEADER || getItemViewType(position) == ITEM_TYPE_FOOTER ? gridLayoutManager.getSpanCount() : 1;
					}
				});
			}
		}
	...
	public int getItemCount() {
		return getRealItemCount() + (isHasHeader() ? 1 : 0) + (isHasFooter() ? 1 : 0);
	}
...
////		super.clear();
//		getList().clear();
//		notifyItemRangeRemoved(isHasHeader() ? 1: 0, count - (isHasHeader() ? 1 : 0));
//	}
}
