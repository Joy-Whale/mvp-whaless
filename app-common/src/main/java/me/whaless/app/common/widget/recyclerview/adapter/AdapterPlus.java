...
public abstract class AdapterPlus<T> extends RecyclerView.Adapter<ViewHolderPlus<T>> {
	...
	public void insertRange(List<T> list, boolean anim) {
		insertRange(getItemCount(), list, anim);
	}
	...
	public void deleteRange(List<T> list) {
		int index = getList().indexOf(list.get(0));
		if (index < 0)
			return;
	...
	public synchronized void clear() {
		if (FieldUtils.isEmpty(mList))
			return;
		mList.clear();
		// try fix bug: java.lang.IllegalStateException: Cannot call this method while RecyclerView is computing a layout or scrolling RecyclerView
		notifyDataSetChanged();
	}
