...
public class AppKeeper {
	...
	public File createAudioFile() {
		String path = getCachePath() + "/voice/" + System.currentTimeMillis() + ".pcm";
	...
	public File createAudioAacFile(){
		String path = getCachePath() + "/voice/" + System.currentTimeMillis() + ".aac";
		File f = new File(path);
		if (f.exists()) {
			f.delete();
		}
